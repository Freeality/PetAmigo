...
- (void)testValidateStringNomeInvalidoNaoPassa {
...
- (void)testNomeInvalidoContaDeveSerNil {
    
    Conta *conta = [[Conta alloc] initWithNome:@"no" Email:EMAIL_PEDRO eSenha:SENHA_TEXT1];
    XCTAssertNil(conta);
}

- (void)testEmailInvalidoContaDeveSerNil {
    Conta *conta = [[Conta alloc] initWithNome:NOME_PEDRO Email:@"email" eSenha:SENHA_TEXT1];
    XCTAssertNil(conta);
}
